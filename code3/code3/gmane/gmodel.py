...
#https://www.youtube.com/watch?v=wSpl1-7afAk&list=PLyZB5ywlnsu81uJdbY8QGcxlRqwB3LBLc&index=2&ab_channel=freeCodeCampConcepts
...
def parseheader(hdr, allsenders=None):
    ...
    z = re.findall('\nSubject: (.*)\n', hdr)
    subject = z[0].strip().lower() if len(z) >= 1 else None
    z = re.findall('\nMessage-ID: (.*)\n', hdr)
    guid = z[0].strip().lower() if len(z) >= 1 else None
...
for message_row in cur_1:
    ...
    sender_id = senders.get(sender)
    subject_id = subjects.get(subject)
    guid_id = guids.get(guid)
