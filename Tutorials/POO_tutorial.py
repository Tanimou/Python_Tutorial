...
class Item(ABC):
    ...
    ):  ## we can also initialise a paramater to its default value
...
class Phone(Item):
    ...
    def __init__(self, name: str, price: float, quantity=0, broken_phones=0):
        ...
        assert (
            broken_phones >= 0
        ), f"Broken phones {broken_phones} is not a valid value!"
...
)  ##so python passes item1 as a first parameter everytime you call calculate_total_price
