...
def scrapping():
    ...
    Prospect_list, Week_list, Despatched_list, Assayed_list, ASS_Pending_list, var, var2, var3, var4, str2, str1,AssaysPen_toweek = Openexcelfile(
        filepath)
   # listweek = list(Week_list)
    ...
    def update(value):
        ...
        if value not in Assayed_list:
            ...
            ASS_Pending_list[value] = Despatched_list[value] - \
                Assayed_list[value]
    ...
    x = threading.Thread(target=create_components, args=(
        var, var2, var3, var4, Prospect_list, update, str2, str1, Week_list, update2, AssaysPen_toweek))
    x.start()
...
def Openexcelfile(filepath):
    with open_workbook(filepath) as wb:
       ...
       # print(Week_list)
        ...
        listweek = list(Week_list)
       # print(listweek)
        scrapping_prospect(wb, Prospect_list, Despatched_list,
                           Assayed_list, listweek, AssaysPen_toweek)
    ...
    for name in dico:
        for dd in dico:
            if name[0] == dd[0] and name[1] == dd[1] and dd[2] <= name[2]:
                dico2[(name[0], name[1], int(name[2]))] = dico2.get(
                    (name[0], name[1], int(name[2])), 0)+dico[(dd[0], dd[1], dd[2])]
...
def find_max(dico,prospect,sample_type,weeknum):
    ...
    for name in dico:
        ...
        if name[0] == prospect and name[1] == sample_type:
            if name[2] <= weeknum:
               d = [(x, y, z) for x, y, z in dico if x ==
                    prospect and y == sample_type and z <= weeknum]
               return dico[max(d)]
            else:
...
def create_components(var, var2, var3, var4, Prospect_list, update, str2, str1, Week_list, update2,AssaysPen_toweek):
    ...
    week_label, SamplesAssayNumber_label, sampleAssayed_label, SamplesAssayNumberP_label, sampleAssayedP_label, SamplesDespaNumber_label, sampleDespatched_label, sampleNumber_label, SamplesData_label = create_labels( var, var2, var3, var4)

    place_labels(week_label, SamplesAssayNumber_label, sampleAssayed_label, SamplesAssayNumberP_label, sampleAssayedP_label,
                 SamplesDespaNumber_label, sampleDespatched_label, sampleNumber_label, SamplesData_label, combo, combo1)

    create_table(prosp=str2, weekly=str1, lw=Week_list[str1.get()],AssayedPW_list=AssaysPen_toweek)
